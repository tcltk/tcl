    ...
    Tcl_Obj **attributePtrPtr)	/* A pointer to return the object with. */
{
    ...
    result = getattrlist(native, &alist, &finfo.info_length, sizeof(fileinfobuf)-offsetof(fileinfobuf,info_length), 0);
				...
				/* Stat info for source file */
{
	...
	if (getattrlist(src, &alist, &finfo.info_length, sizeof(fileinfobuf)-offsetof(fileinfobuf,info_length), 0)) {
	    return TCL_ERROR;
	}
    ...
    Tcl_GlobTypeData *types)	/* Type description to match against. */
{
    ...
    alist.bitmapcount = ATTR_BIT_MAP_COUNT;
    alist.commonattr = ATTR_CMN_FNDRINFO;
    if (getattrlist(pathName, &alist, &finfo.info_length, sizeof(fileinfobuf)-offsetof(fileinfobuf,info_length), 0)) {
	return 0;
    }
