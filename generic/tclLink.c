				 ...
				 * may have TCL_LINK_READ_ONLY OR'ed in. */
{
    ...
    if (linkPtr != NULL) {
	Tcl_PrintfResult(interp, "variable '%s' is already linked", varName);
	return TCL_ERROR;
    }
    ...
    Tcl_Size size)		/* Size of C variable array, >1 if array */
{
    ...
    if (size < 1) {
	Tcl_PrintfResult(interp, "wrong array size given");
	return TCL_ERROR;
    }
    ...
    default:
	LinkFree(linkPtr);
	Tcl_PrintfResult(interp, "bad linked array variable type");
	return TCL_ERROR;
    }
