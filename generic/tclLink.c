...
enum LinkFlags {
    LINK_READ_ONLY = 1,		/* Errors should be generated if Tcl script
				 * attempts to write tje variable. */
    LINK_BEING_UPDATED = 2,	/* A call to Tcl_UpdateLinkedVar is in progress
				 * for this variable, so trace callbacks on the
				 * variable should be ignored. */
    LINK_ALLOC_ADDR = 4,	/* linkPtr->addr was allocated on the heap. */
    LINK_ALLOC_LAST = 8		/* linkPtr->valueLast.p was allocated on the heap. */
};
