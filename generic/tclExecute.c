...
typedef struct {
    ...
    int traceInstructions;	/* Whether we are doing instruction-level
				 * tracing or not. */
    char cmdNameBuf[21];	/* Space to store the command name across an
				 * invoke. */
#endif
...
/*
 * Macros used to trace instruction execution. The macros TRACE and O2S are
 * only used inside TclNRExecuteByteCode. O2S is only used in TRACE* calls to
...
#   define TRACE(...) \
	...
	fprintf(stdout, "%2" SIZEd ": %2" SIZEd	" (%" SIZEd ") %s ",	\
		iPtr->numLevels, CURR_DEPTH, PC_REL, GetOpcodeName(pc));\
...
#define TCL_DTRACE_INST_NEXT() \
	...
	if (TCL_DTRACE_INST_DONE_ENABLED()) {				\
		...
		TCL_DTRACE_INST_START(curInstName, CURR_DEPTH, tosPtr);	\
	    }								\
...
#ifdef TCL_COMPILE_DEBUG
    TD->cmdNameBuf[0] = 0;
    TD->traceInstructions = (tclTraceExec >= TCL_TRACE_BYTECODE_EXEC_INSTRUCTIONS);
#endif
...
#define traceInstructions (TD->traceInstructions)
  ...
  cleanup0:
	...
	if (tosPtr > initTosPtr) {
	    ...
	    TRACE_APPEND_OBJ(iPtr->objResultPtr);
	    if (traceInstructions) {
		fprintf(stdout, "\n");
	    }
#endif
...
/*TODO: Consider more untangling here; merge with LOAD and STORE ? */
   ...
   case INST_JUMP_TABLE:
	...
	TRACE("\"%.20s\" \"%.20s\" \"%.20s\" \"%.20s\" => ", O2S(valuePtr),
	...
	*/
	...
	for (i = 0;  i < numLists;  i++) {
		...
		CACHE_STACK_INFO();
		TRACE_APPEND("ERROR converting list %" SIZEd ", \"%.30s\": ",
			i, O2S(listPtr));
		TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
		goto gotError;
	    }
	...
	if (iterNum < iterMax) {
		...
		for (j = 0;  j < numVars;  j++) {
			...
			if (TclPtrSetVarIdx(interp, varPtr, NULL, NULL, NULL,
			    ...
			    CACHE_STACK_INFO();
			    TRACE_APPEND("ERROR init. index temp %" SIZEd ": ",
				    varIndex);
			    TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
			    goto gotError;
			}
	...
	*(++catchTop) = (Tcl_Obj *)INT2PTR(CURR_DEPTH);
	...
	if (Tcl_DictObjSize(interp, dictPtr, &size) != TCL_OK) {
	    TRACE_APPEND("ERROR verifying dictionary nature of \"%.30s\": ",
		    O2S(dictPtr));
	    TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
	    goto gotError;
	}
	...
	if (Tcl_DictObjGet(interp, dictPtr, OBJ_AT_TOS,
		&objResultPtr) != TCL_OK) {
	    TRACE_APPEND("ERROR reading leaf dictionary key \"%.30s\": ",
		    O2S(OBJ_AT_TOS));
	    TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
	    goto gotError;
	}
	if (!objResultPtr) {
	    ...
	    Tcl_SetErrorCode(interp, "TCL", "LOOKUP", "DICT",
		    TclGetString(OBJ_AT_TOS), (char *)NULL);
	    CACHE_STACK_INFO();
	    TRACE_APPEND("ERROR leaf dictionary key \"%.30s\" absent: ",
		    O2S(OBJ_AT_TOS));
	    TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
	    goto gotError;
	}
	...
	if (result != TCL_OK) {
	    ...
	    TRACE_APPEND("ERROR updating dictionary: ");
	    TRACE_APPEND_OBJ(Tcl_GetObjResult(interp));
	    goto checkForCatch;
	}
