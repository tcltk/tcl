...
#define cmdNameBuf	(TD->cmdNameBuf)
  ...
  cleanup0:
    ...
    case INST_EXPR_STK: {
	TRACE(" => YIELD TO COMPILED EXPRESSION\n");
	bcFramePtr->data.tebc.pc = (char *) pc;
	iPtr->cmdFramePtr = bcFramePtr;
	DECACHE_STACK_INFO();
 ...
 * GetOpcodeName --
 *
 *	This procedure is called by the TRACE macro used in
 *	TclNRExecuteByteCode when debugging. It returns the name of the
