    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	Tcl_PrintfResult(interp, "channel \"%s\" wasn't opened for writing",
  ...
  error:
	...
	Tcl_PrintfResult(interp, "error writing \"%s\": %s",
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if (!TclChanCaughtErrorBypass(interp, chan)) {
	    Tcl_PrintfResult(interp, "error flushing \"%s\": %s",
		    TclGetString(chanObjPtr), Tcl_PosixError(interp));
	}
	TclChannelRelease(chan);
	return TCL_ERROR;
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	Tcl_PrintfResult(interp, "channel \"%s\" wasn't opened for reading",
		TclGetString(chanObjPtr));
	return TCL_ERROR;
    }
	...
	if (!Tcl_Eof(chan) && !Tcl_InputBlocked(chan)) {
	    ...
	    if (!TclChanCaughtErrorBypass(interp, chan)) {
		Tcl_PrintfResult(interp, "error reading \"%s\": %s",
			TclGetString(chanObjPtr), Tcl_PosixError(interp));
	    }
	    code = TCL_ERROR;
	    goto done;
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if ((TclGetWideIntFromObj(NULL, objv[i], &toRead) != TCL_OK)
		|| (toRead < 0)) {
	    Tcl_PrintfResult(interp, "expected non-negative integer but got \"%s\"",
		    TclGetString(objv[i]));
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if (!TclChanCaughtErrorBypass(interp, chan)) {
	    Tcl_PrintfResult(interp, "error during seek on \"%s\": %s",
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	case EXEC_ENCODING:
	    if (++skip >= objc) {
		Tcl_PrintfResult(interp, "No value given for option -encoding");
		return TCL_ERROR;
	    } else {
	...
	if (Tcl_ReadChars(chan, resultPtr, -1, 0) == TCL_IO_FAILURE) {
	    ...
	    if (!TclChanCaughtErrorBypass(interp, chan)) {
		Tcl_PrintfResult(interp, "error reading output from command: %s",
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	case SKT_ASYNC:
	    if (server == 1) {
		Tcl_PrintfResult(interp, "cannot set -async option for server sockets");
		return TCL_ERROR;
	    }
	    async = 1;
	    break;
	...
	if (myport != 0) {
	    Tcl_PrintfResult(interp, "option -myport is not valid for servers");
	    return TCL_ERROR;
	}
    } else if (a < objc) {
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	Tcl_PrintfResult(interp, "error during truncate on \"%s\": %s",
		TclGetString(objv[1]), Tcl_PosixError(interp));
	return TCL_ERROR;
    }
