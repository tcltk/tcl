...
enum ChannelStateFlags {
    ...
    CHANNEL_ENCODING_ERROR = 1<<15,
				/* Channel encountered an encoding error. */
    CHANNEL_RAW_MODE = 1<<16,	/* Notes that the Raw API is being used. */
    CHANNEL_LINEBUFFERED = 1<<17,
				/* Output to the channel must be flushed after
				 * every newline. */
    CHANNEL_UNBUFFERED = 1<<18,	/* Output to the channel must always be
				 * flushed immediately. */
    CHANNEL_INCLOSE = 1<<19,	/* Channel is currently being closed. Its
				 * structures are still live and usable, but it
				 * may not be closed again from within the
				 * close handler. */
    CHANNEL_CLOSEDWRITE = 1<<21	/* Channel write side has been closed. No
				 * further Tcl-level write IO on the channel is
				 * allowed. */
};
