    ...
    const char *nsNameStr)
{
	...
	if (hPtr) {
	    Tcl_PrintfResult(interp, "can't create object \"%s\": "
		    "command already exists with that name", nameStr);
    ...
    int result)
{
	...
	Tcl_PrintfResult(interp, "object deleted in constructor");
    ...
    const char *targetNamespaceName)
{
    ...
    if (IsRootClass(oPtr)) {
	Tcl_PrintfResult(interp, "may not clone the class of classes");
				 ...
				 * the normal case). */
{
	...
	if (contextPtr->index >= contextPtr->callPtr->numChain) {
	    Tcl_PrintfResult(interp, "no valid method implementation");
	    Tcl_SetErrorCode(interp, "TCL", "LOOKUP", "METHOD",
		    TclGetString(methodNamePtr), (char *)NULL);
	    TclOODeleteContext(contextPtr);
	    return TCL_ERROR;
				 ...
				 * exactly the name of its public command. */
{
  ...
  notAnObject:
    Tcl_PrintfResult(interp, "%s does not refer to an object",
	    TclGetString(objPtr));
