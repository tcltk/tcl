    ...
    Tcl_Obj *const *objv)
{
	...
	if (Tcl_GetIndexFromObj(interp, listv[listc-1], methodNames,
	    ...
	    Tcl_PrintfResult(interp, "chan handler \"%s initialize\" returned %s",
    ...
    if (!mode) {
	Tcl_PrintfResult(interp, "chan handler \"%s\" makes the channel inaccessible",
		TclGetString(cmdObj));
	goto error;
    }
    ...
    Tcl_Obj **resultObjPtr)	/* NULL'able */
{
	...
	if (result == TCL_OK) {
		...
		Tcl_PrintfResult(rtPtr->interp,
			"chan handler returned bad code: %d", result);
		Tcl_LogCommandInfo(rtPtr->interp, cmdString, cmdString, cmdLen);
		Tcl_DecrRefCount(cmd);
		result = TCL_ERROR;
	    }
	    Tcl_AppendPrintfToErrorInfo(rtPtr->interp,
