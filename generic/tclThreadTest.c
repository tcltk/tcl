    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if (result == TCL_OK) {
	    ...
	    Tcl_PrintfResult(interp, "cannot join thread %" TCL_LL_MODIFIER "d",
		    (long long)id);
	}
	return result;
    }
    ...
    int joinable)		/* Flag, joinable thread or not */
{
	...
	Tcl_MutexUnlock(&threadMutex);
	Tcl_PrintfResult(interp, "cannot create a new thread");
	return TCL_ERROR;
    }
    ...
    int wait)			/* If 1, we block for the result. */
{
    ...
    if (!found) {
	Tcl_MutexUnlock(&threadMutex);
	Tcl_PrintfResult(interp, "invalid thread id");
	return TCL_ERROR;
    }
