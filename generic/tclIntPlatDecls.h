...
/* 10 */
TCLAPI Tcl_DirEntry *	TclpReaddir(TclDIR *dir);
/* 11 */
TCLAPI struct tm *	TclpLocaltime_unix(const time_t *clock);
/* 12 */
TCLAPI struct tm *	TclpGmtime_unix(const time_t *clock);
/* 13 */
...
/* 1 */
TCLAPI void		TclWinConvertWSAError(DWORD errCode);
/* 2 */
...
TCLAPI TclFile		TclpOpenFile(const char *fname, int mode);
/* 20 */
TCLAPI void		TclWinAddProcess(void *hProcess, Tcl_Size id);
/* 21 */
TCLAPI char *		TclpInetNtoa(struct in_addr addr);
/* 22 */
TCLAPI TclFile		TclpCreateTempFile(const char *contents);
