				 ...
				 * created but it will never get any data. */
{
    ...
    TclFile inputFile = NULL;	/* If != NULL, gives file to use as input for
				 ...
				 * or <@). */
    bool inputClose = false;	/* If true, then inputFile should be
				 * closed when cleaning up. */
    bool inputRelease = false;
    TclFile outputFile = NULL;	/* Writable file for output from last command
				 * in pipeline (could be file or pipe). NULL
				 * means use stdout. */
    bool outputClose = false;	/* If true, then outputFile should be
				 * closed when cleaning up. */
	...
	*errFilePtr = NULL;
	...
	case '<':
		...
		inputLiteral = NULL;
		inputFile = FileForRedirect(interp, p, true, argv[i], nextArg,
