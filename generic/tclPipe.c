    ...
    int *releasePtr)
{
    ...
    *skipPtr = 1;
	...
	if (file == NULL) {
		...
		Tcl_PrintfResult(interp, "channel \"%s\" wasn't opened for %s",
	...
	*releasePtr = 1;
	...
	if (file == NULL) {
	    Tcl_PrintfResult(interp, "couldn't %s file \"%s\": %s",
		    (writing ? "write" : "read"), spec, Tcl_PosixError(interp));
	    return NULL;
	}
	*closePtr = 1;
    }
  ...
  badLastArg:
    Tcl_PrintfResult(interp, "can't specify \"%s\" as last word in command",
	    arg);
				 ...
				 * stderr output. */
{
	...
	Tcl_PrintfResult(interp, "child process exited abnormally");
    }
    return result;
}

	...
	*errFilePtr = NULL;
	...
	case '|':
		...
		if ((i == (lastBar + 1)) || (i == (argc - 1))) {
		    Tcl_PrintfResult(interp, "illegal use of | or |& in command");
		    Tcl_SetErrorCode(interp, "TCL", "OPERATION", "EXEC",
				 ...
				 * TCL_STDERR, and TCL_ENFORCE_MODE. */
{
	...
	if ((flags & TCL_STDOUT) && (outPipe == NULL)) {
	    Tcl_PrintfResult(interp, "can't read output from command:"
	...
	if ((flags & TCL_STDIN) && (inPipe == NULL)) {
	    Tcl_PrintfResult(interp, "can't write input to command:"
    ...
    if (channel == NULL) {
	Tcl_PrintfResult(interp, "pipe for command could not be created");
