    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	for (asyncPtr = firstHandler; asyncPtr != NULL;
		...
		if (Tcl_CreateThread(&threadID, AsyncThreadProc,
		    ...
		    Tcl_PrintfResult(interp, "cannot create thread");
		    Tcl_MutexUnlock(&asyncTestMutex);
		    return TCL_ERROR;
		}
		break;
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if (info.isNativeObjectProc == 0) {
	    ...
	    Tcl_PrintfResult(interp, "Invalid isNativeObjectProc value %d",
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	if (sscanf(Tcl_GetString(objv[2]), "%d", &id) != 1) {
	    Tcl_PrintfResult(interp, "bad command token \"%s\"",
    ...
    Tcl_Obj *const *objv)	/* Parameter vector */
{
	...
	if (strcmp(Tcl_GetString(objv[4]), "shorten")) {
	    Tcl_PrintfResult(interp, "bad flag");
	    return TCL_ERROR;
	}
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	if (strcmp(Tcl_GetString(objv[2]), "staticsmall") == 0) {
	    ...
	    Tcl_PrintfResult(interp, "bad gresult option \"%s\": must be %s",
		    Tcl_GetString(objv[2]),
    ...
    Tcl_Obj *const *objv)
{
	...
	if (prefixLen < 0) {
	    Tcl_PrintfResult(interp, "prefixLen must be non-negative");
	    return TCL_ERROR;
	}
    }
	...
	Tcl_PrintfResult(interp, "%s wrote past output buffer",
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	Tcl_SetObjResult(interp, Tcl_NewIntObj(
		Tcl_GetEncodingNulLength(encoding)));
	Tcl_FreeEncoding(encoding);
	break;
    case ENC_EXTTOUTF:
	...
	if (string != Tcl_DStringValue(&ds)) {
	    Tcl_DStringFree(&ds);
	    Tcl_PrintfResult(interp, "Returned pointer not same as DString value");
	    return TCL_ERROR;
	}
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
    ...
    flags = 0;
    if (objc == 3) {
	const char *globalOpt = Tcl_GetString(objv[2]);
	if (strcmp(globalOpt, "global") != 0) {
	    Tcl_PrintfResult(interp, "bad value \"%s\": must be %s",
		    globalOpt, "global");
	    return TCL_ERROR;
	}
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
    ...
    Tcl_PrintfResult(interp, "This is a result");
    double exprResult;
    int result = Tcl_ExprDoubleObj(interp, objv[1], &exprResult);
    if (result != TCL_OK) {
	return result;
    }
    Tcl_AppendPrintfResult(interp, ": %.15g", exprResult);
    return TCL_OK;
}

/*
    ...
    Tcl_Obj *const *objv)	/* The argument objects. */
{
	...
	if (contents == NULL) {
	    Tcl_PrintfResult(interp, "could not read link \"%s\": %s",
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	Tcl_PrintfResult(interp, "bad option \"%s\": should be %s",
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	if (Tcl_GetIntFromObj(interp, objv[i++], &size) == TCL_ERROR) {
	    Tcl_PrintfResult(interp, "wrong size value");
	    return TCL_ERROR;
	}
	...
	if (i < objc) {
		...
		Tcl_PrintfResult(interp, "wrong address value");
		return TCL_ERROR;
	    }
	} else {
	    addr = 0;
    ...
    Tcl_Obj *const *objv)	/* The argument objects. */
{
    ...
    if (w <= 0) {
	Tcl_PrintfResult(interp, "argument must be positive");
	return TCL_ERROR;
    }
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
  ...
  endOfForLoop:
	...
	if (objc > 2 && (cflags&REG_EXPECT) && indices) {
	    ...
	    if (value == NULL) {
		Tcl_PrintfResult(interp, "couldn't set variable \"%s\"",
			varName);
		return TCL_ERROR;
	    }
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	*platform = TCL_PLATFORM_WINDOWS;
	...
	Tcl_PrintfResult(interp, "unsupported platform: should be one of %s",
		"unix, or windows");
	return TCL_ERROR;
    }
    return TCL_OK;
    ...
    Tcl_Obj *const *objv)	/* The argument objects. */
{
	...
	Tcl_PrintfResult(interp, "Unknown subcommand \"%s\".", subcmd);
	result = TCL_ERROR;
	goto end;
    }
    ...
    Tcl_Obj *const *objv)	/* Additional args. */
{
	...
	if (objc != 3) {
	    Tcl_PrintfResult(interp, "channel name required");
	    return TCL_ERROR;
	}
	Tcl_SetObjResult(interp, Tcl_NewIntObj(
	...
	if (strcmp(Tcl_GetString(objv[3]), "-command") != 0) {
	    Tcl_PrintfResult(interp, "bad argument \"%s\": should be \"%s\"",
		    Tcl_GetString(objv[3]), "-command");
	    return TCL_ERROR;
	}
    ...
    Tcl_PrintfResult(interp, "bad option \"%s\": should be %s", cmdName,
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	if (strcmp(Tcl_GetString(objv[3]), "readable") == 0) {
	    ...
	    Tcl_PrintfResult(interp, "bad event name \"%s\": must be %s",
	...
	if (index < 0) {
	    Tcl_PrintfResult(interp, "bad event index %d: must be nonnegative",
		    index);
	    return TCL_ERROR;
	}
	...
	if (esPtr == NULL) {
	    Tcl_PrintfResult(interp, "bad event index %d: out of range", index);
	    return TCL_ERROR;
	}
    ...
    Tcl_PrintfResult(interp, "bad command %s, must be one of %s",
    ...
    Tcl_Obj *const *objv)	/* Additional args. */
{
	...
	if (NULL == hChannel) {
	    Tcl_PrintfResult(interp, "unknown channel: %s", Tcl_GetString(objv[2]));
	    return TCL_ERROR;
	}
	statePtr = (TcpState *)Tcl_GetChannelInstanceData(hChannel);
	if (NULL == statePtr) {
	    Tcl_PrintfResult(interp, "No channel instance data: %s", Tcl_GetString(objv[2]));
	    return TCL_ERROR;
	}
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	 ...
	 */
    insufArgs:
	Tcl_PrintfResult(interp, "insufficient arguments");
	return TCL_ERROR;
    }
    ...
    Tcl_Obj *const *objv)
{
	...
	if (first != result) {
	    Tcl_PrintfResult(interp,
		    "Tcl_UtfNext is not supposed to read src[-1]");
	    return TCL_ERROR;
	}
    }
    p = tobetested;
    ...
    Tcl_Obj *const *objv)	/* Parameter vector */
{
    ...
    if (status != TCL_OK) {
	Tcl_PrintfResult(interp, "operation not available");
	return status;
    }
    for (i=0 ; i<4 ; ++i) {
    ...
    Tcl_Obj *const *objv)
{
	...
	Tcl_PrintfResult(interp, "non-zero initial size");
	...
	if (!isNew) {
	    Tcl_PrintfResult(interp, "%d creation problem", i);
	    Tcl_DeleteHashTable(&hash);
	    return TCL_ERROR;
	}
	...
	Tcl_PrintfResult(interp, "unexpected maximal size");
	Tcl_DeleteHashTable(&hash);
	return TCL_ERROR;
    }
	...
	if (hPtr == NULL) {
	    Tcl_PrintfResult(interp, "%d lookup problem", i);
	    Tcl_DeleteHashTable(&hash);
	    return TCL_ERROR;
	}
	if (PTR2INT(Tcl_GetHashValue(hPtr)) != i+42) {
	    Tcl_PrintfResult(interp, "%d value problem", i);
	    Tcl_DeleteHashTable(&hash);
	    return TCL_ERROR;
	}
    ...
    if (hash.numEntries != 0) {
	Tcl_PrintfResult(interp, "non-zero final size");
	Tcl_DeleteHashTable(&hash);
	return TCL_ERROR;
    }
...
#define RESOLVER_KEY "testInterpResolver"
	...
	if (interp == NULL) {
	    Tcl_PrintfResult(interp, "provided interpreter not found");
	    return TCL_ERROR;
	}
    }
	...
	if (!Tcl_RemoveInterpResolvers(interp, RESOLVER_KEY)) {
	    Tcl_PrintfResult(interp, "could not remove the resolver scheme");
	    return TCL_ERROR;
	}
    }
    return TCL_OK;
    ...
    Tcl_Obj *const *objv)
{
	...
	Tcl_PrintfResult(interp, "Failed to create channel");
	return TCL_ERROR;
    }
    Tcl_RegisterChannel(interp, chan);
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
	...
	if (len > slen) {
	    Tcl_PrintfResult(interp, "Passed length %" TCL_SIZE_MODIFIER
...
PostInitRaiseErrorProc(
    ...
    TCL_UNUSED(void *))
{
    Tcl_PrintfResult(interp, "PostInit test error raised");
    return TCL_ERROR;
}
    ...
    Tcl_Obj *const objv[])	/* Arguments. */
{
    ...
    if (ret != TCL_OK) {
	Tcl_PrintfResult(interp, "Could not %s callback %s",
    ...
    Tcl_Obj *const *objv)	/* Arguments. */
{
    ...
    Tcl_PrintfResult(interp, "GetProcessHandleCount failed");
    return TCL_ERROR;
}

/*
