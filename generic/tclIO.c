    ...
    int mode)			/* The access mode to drop from the channel */
{
  ...
  error:
    if (interp != NULL) {
	Tcl_PrintfResult(interp, "Tcl_RemoveChannelMode error: %s. Channel: \"%s\"",
    ...
    ChannelState *statePtr)	/* The channel state to check. */
{
    ...
    Tcl_SetErrno(EINVAL);
    if (interp) {
	Tcl_PrintfResult(interp, "unable to access channel: invalid channel");
    }
    return 1;
}

				 ...
				 * asynchronous flush callback. */
{
	...
	if (written < 0) {
		...
		if (interp != NULL && !TclChanCaughtErrorBypass(interp,
			(Tcl_Channel) chanPtr)) {
		    Tcl_PrintfResult(interp, "%s", Tcl_PosixError(interp));
		}
    ...
    const char *newValue)	/* New value for option. */
{
	...
	if (interp) {
	    Tcl_PrintfResult(interp, "unable to set channel options: "
