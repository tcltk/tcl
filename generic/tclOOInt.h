...
typedef struct ProcedureMethod {
    int version;		/* Version of this structure. Currently must
				 * be TCLOO_PROCEDURE_METHOD_VERSION_0. */
...
enum ProcedureMethodVersions {
    TCLOO_PROCEDURE_METHOD_VERSION_0 = 0
};
#define TCLOO_PROCEDURE_METHOD_VERSION TCLOO_PROCEDURE_METHOD_VERSION_0

/*
 * Flags for use in a ProcedureMethod.
 */
enum ProcedureMethodFlags {
    USE_DECLARER_NS = 0x80	/* When this is set, the method will use the
				 * namespace of the object or class that
				 * declared it (or the clone of it, if it was
				 * from such that the implementation of the
				 * method came to the particular use) instead
				 * of the namespace of the object on which the
				 * method was invoked. This flag must be
				 * distinct from all others that are associated
				 * with methods. */
};
 ...
 *
 * Note that "non-exported" methods are those that don't have any SCOPE_FLAGS
 * set.
 */
enum CallChainFlags {
