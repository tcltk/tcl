    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
	...
	Tcl_PrintfResult(interp, "must specify either file name or prefix");
	...
	if (prefix != NULL) {
		...
		Tcl_PrintfResult(interp, "cannot figure out prefix for %s",
			fullFileName);
		Tcl_SetErrorCode(interp, "TCL", "OPERATION", "LOAD",
	...
	if (iPtr->legacyResult && *(iPtr->legacyResult) && !iPtr->legacyFreeProc) {
	     ...
	     */
	    Tcl_PrintfResult(target, "this extension is compiled for Tcl 8.x");
    ...
    Tcl_Obj *const *objv)	/* Argument objects. */
{
  ...
  endOfForLoop:
	...
	Tcl_PrintfResult(interp, "file \"%s\" has never been loaded",
		fullFileName);
