				 ...
				 * required. */
{
	...
	case 's':
	    ...
	    invalidFieldSize:
		// ch is probably 'n', 's' or '[', but be careful anyway.
	...
	case '[':
	    ...
	    break;
	badSet:
	    Tcl_PrintfResult(interp, "unmatched [ in format string");
	    Tcl_SetErrorCode(interp, "TCL", "FORMAT", "BRACKET", (char *)NULL);
	    goto error;
	default:
	    // Do NOT know what character we've got; may be non-ASCII.
	    buf[Tcl_UniCharToUtf(ch, buf)] = '\0';
	    Tcl_PrintfResult(interp, "bad scan conversion character \"%s\"",
		    buf);
  ...
  badIndex:
    if (gotXpg) {
	Tcl_PrintfResult(interp, "\"%%n$\" argument index out of range");
