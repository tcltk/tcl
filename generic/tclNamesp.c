				 ...
				 * function should be called. */
{
    ...
    if (*name == '\0') {
	Tcl_PrintfResult(interp, "can't create namespace \"\": "
...
#define INIT_EXPORT_PATTERNS 5
	...
	Tcl_PrintfResult(interp, "invalid export pattern \"%s\": "
		"pattern can't specify a namespace", pattern);
				 ...
				 * conflicts with an existing one. */
{
    ...
    if (strlen(pattern) == 0) {
	Tcl_PrintfResult(interp, "empty import pattern");
				 ...
				 * TCL_LEAVE_ERR_MSG flags. */
{
	...
	Tcl_PrintfResult(interp, "unknown namespace \"%s\"", name);
				 ...
				 * TCL_GLOBAL_ONLY is ignored. */
{
    ...
    if (flags & TCL_LEAVE_ERR_MSG) {
	Tcl_PrintfResult(interp, "unknown command \"%s\"", name);
    ...
    Tcl_Namespace **nsPtrPtr)	/* Result namespace pointer goes here. */
{
	...
	if ((name[0] == ':') && (name[1] == ':')) {
	    Tcl_PrintfResult(interp, "namespace \"%s\" not found", name);
	} else {
	    /*
	    ...
	    Tcl_PrintfResult(interp, "namespace \"%s\" not found in \"%s\"",
