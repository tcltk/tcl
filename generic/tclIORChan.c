    ...
    Tcl_Obj *const *objv)
{
	...
	Tcl_PrintfResult(interp, "chan handler \"%s\" lacks a \"%s\" method",
		TclGetString(cmdObj), "read");
	goto error;
    }

    if ((mode & TCL_WRITABLE) && !HAS(methods, METH_WRITE)) {
	Tcl_PrintfResult(interp, "chan handler \"%s\" lacks a \"%s\" method",
		TclGetString(cmdObj), "write");
	goto error;
    }

    if (!IMPLIES(HAS(methods, METH_CGET), HAS(methods, METH_CGETALL))) {
	Tcl_PrintfResult(interp, "chan handler \"%s\" supports \"%s\" but not \"%s\"",
		TclGetString(cmdObj), "cget", "cgetall");
	goto error;
    }

    if (!IMPLIES(HAS(methods, METH_CGETALL), HAS(methods, METH_CGET))) {
	Tcl_PrintfResult(interp, "chan handler \"%s\" supports \"%s\" but not \"%s\"",
		TclGetString(cmdObj), "cgetall", "cget");
	goto error;
    }
    ...
    Tcl_Obj *const *objv)
{
    ...
    if (hPtr == NULL) {
	Tcl_PrintfResult(interp, "can not find reflected channel named \"%s\"",
		chanId);
    ...
    if (events == 0) {
	Tcl_PrintfResult(interp, "bad event list: is empty");
	return TCL_ERROR;
    }
    ...
    Tcl_DString *dsPtr)		/* String to place the result into */
{
	...
	Tcl_ResetResult(interp);
	Tcl_PrintfResult(interp, "Expected list with even number of elements, "
    ...
    Tcl_Obj **resultObjPtr)	/* NULL'able */
{
	...
	if (result == TCL_OK) {
	    ...
	    Tcl_AppendPrintfToErrorInfo(rcPtr->interp,
