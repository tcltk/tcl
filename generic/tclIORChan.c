    ...
    Tcl_DString *dsPtr)		/* String to place the result into */
{
    ...
    if (TclListObjGetElements_(interp, resObj, &listc, &listv) != TCL_OK) {
        goto error;
    }
    ...
    int *mask)
{
    ...
    if (TclListObjGetElements_(interp, obj, &listc, &listv) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    TCL_UNUSED(int) /* mask */)
{
	...
	if (InvokeTclMethod(rcPtr, METH_CGETALL, NULL, NULL, &resObj) != TCL_OK){
	    ...
	    if (TclListObjGetElements_(interp, resObj, &listc,
                    &listv) != TCL_OK) {
		Tcl_DecrRefCount(resObj);
		resObj = MarshallError(interp);
		ForwardSetObjError(paramPtr, resObj);
