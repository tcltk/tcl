...
typedef unsigned char uchar;		// Just to keep things shorter...
    ...
    void *arg)
{
    ...
    Tcl_MutexLock(listLockPtr);
    Cache **nextPtrPtr = &firstCachePtr;
    ...
    size_t reqSize)
{
    ...
    Block *blockPtr = NULL;
    size_t size = reqSize + sizeof(Block);
#if RCHECK
    size++;
#endif
    int bucket;
    ...
    void *ptr)
{
    ...
    int bucket = blockPtr->sourceBucket;
    if (bucket == NBUCKETS) {
...
TclThreadAllocObj(void)
{
	...
	size_t numMove = sharedPtr->numObjects;
	if (numMove > 0) {
    ...
    size_t reqSize)
{
    ...
    void *ptr = ((void *) (blockPtr + 1));
#if RCHECK
    ((uchar *)(ptr))[reqSize] = MAGIC;
#endif
    return ptr;
}
...
#if RCHECK
    if (((uchar *) ptr)[blockPtr->blockReqSize] != MAGIC) {
	Tcl_Panic("alloc: invalid block: %p: %x %x %x",
		blockPtr, blockPtr->magicNum1, blockPtr->magicNum2,
		((uchar *) ptr)[blockPtr->blockReqSize]);
    }
#endif
    return blockPtr;
}
    ...
    int bucket)
{
	...
	if (sharedPtr->buckets[bucket].numFree > 0) {
	    ...
	    size_t n = bucketInfo[bucket].numMove;
		...
		Block *blockPtr = sharedPtr->buckets[bucket].firstPtr;
		cachePtr->buckets[bucket].firstPtr = blockPtr;
