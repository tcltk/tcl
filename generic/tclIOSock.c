    ...
    int *portPtr)		/* Return port number */
{
	...
	native = Tcl_UtfToExternalDString(NULL, string, TCL_STRLEN, &ds);
	...
	Tcl_SetObjResult(interp, Tcl_NewStringObj(
		"couldn't open socket: port number too high", TCL_STRLEN));
	return TCL_ERROR;
    }
    return TCL_OK;
}
					 ...
					 * detail, if available. */
{
    ...
    if (host != NULL) {
	native = Tcl_UtfToExternalDString(NULL, host, TCL_STRLEN, &ds);
    }
