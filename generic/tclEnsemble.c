    ...
    Tcl_Obj *const *objv)	/* Option-related arguments. */
{
	...
	case CONF_NAMESPACE:
	    Tcl_PrintfResult(interp, "option -namespace is read-only");
    ...
    Tcl_Command token)		/* What to check for ensemble-ness. */
{
	...
	if (interp != NULL) {
	    Tcl_PrintfResult(interp, "command is not an ensemble");
				 ...
				 * are probably not useful. */
{
	...
	if (token == NULL ||
		...
		Tcl_PrintfResult(interp, "\"%s\" is not an ensemble command",
    ...
    Tcl_Obj *const *objv)
{
  ...
  unknownOrAmbiguousSubcommand:
    ...
    Tcl_PrintfResult(interp, "unknown%s subcommand \"%s\": must be ",
	    (ensemblePtr->flags & TCL_ENSEMBLE_PREFIX ? " or ambiguous" : ""),
	    TclGetString(subObj));
    if (ensemblePtr->subcommandTable.numEntries == 1) {
	Tcl_AppendPrintfResult(interp, "%s", ensemblePtr->subcommandArrayPtr[0]);
    } else {
	Tcl_Size i;
