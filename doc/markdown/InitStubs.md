...
Tcl\_InitStubs - initialize the Tcl stubs mechanism
...
1. Call **Tcl\_InitStubs** in the extension before calling any other Tcl functions.

2. Define the **USE\_TCL\_STUBS** symbol.  Typically, you would include the **-DUSE\_TCL\_STUBS** flag when compiling the extension.
...
**Tcl\_InitStubs** attempts to initialize the stub table pointers and ensure that the correct version of Tcl is loaded.  In addition to an interpreter handle, it accepts as arguments a version number and a Boolean flag indicating whether the extension requires an exact version match or not.  If *exact* is 0, then versions newer than *version* are also accepted. If the*version* ends with **-**, higher major versions are accepted as well, otherwise the major version must be the same as in *version*. 1 means that only the specified *version* is accepted. *version* can be any construct as described for [package require][package] (**PACKAGE** manual page in the section **REQUIREMENT**). Multiple requirement strings like with [package require][package] are not supported.  **Tcl\_InitStubs** returns a string containing the actual version of Tcl satisfying the request, or NULL if the Tcl version is not accepted, does not support stubs, or any other error condition occurred.
