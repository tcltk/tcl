...
[**-profile** *profile*]{version="TIP656"}
: [Specifies the encoding profile to be used on the channel. The encoding transforms in use for the channel's input and output will then be subject to the rules of that profile. Any failures will result in a channel error. See **PROFILES** in the documentation of the [encoding] command for details about encoding profiles.]{version="TIP656"}
