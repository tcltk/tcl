...
The **Tcl\_ChannelType** version currently supported is **TCL\_CHANNEL\_VERSION\_5**.  See [Tcl\_CreateChannel][CrtChannel] for details.

**Tcl\_StackChannel** stacks a new *channel* on an existing channel with the same name that was registered for *channel* by [Tcl\_RegisterChannel][OpenFileChnl].
