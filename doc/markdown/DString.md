...
**Tcl\_DStringToObj** returns a [Tcl\_Obj][Object] containing the value of the dynamic string given by *dsPtr*. It does this by moving a pointer from *dsPtr* to a newly allocated [Tcl\_Obj][Object] and reinitializing to dynamic string to an empty string. This saves the cost of allocating new memory and copying the string. Since the dynamic string is reinitialized, there is no need to further call **Tcl\_DStringFree** on it and it can be reused without calling **Tcl\_DStringInit**. The returned [Tcl\_Obj][Object] has a reference count of 0. The caller must ensure that the dynamic string stored in *dsPtr* is encoded in TUTF-8. 
