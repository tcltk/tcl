...
**tcl\_library**
: This variable holds the name of a directory containing the system library of Tcl scripts and encodings. The value of this variable is returned by the [info library][info] command. See the **library** manual entry for details of the facilities provided by the Tcl script library. Normally each application or package will have its own application-specific script library in addition to the Tcl script library; each application should set a global variable with a name like **$***app***\_library** (where *app* is the application's name) to hold the network file name for that application's library directory. The initial value of **tcl\_library** is set when an interpreter is created by searching several different directories until one is found that contains an appropriate Tcl startup script. If an application has already set the **tcl\_library** variable in the interpreter, it is used directly. Otherwise, if the **TCL\_LIBRARY** environment variable exists, then the directory it names is used to initialize **tcl\_library**. In both cases, no further directories are searched. If neither of the above is set, Tcl will search several other directories in the following order: the ZipFS archive attached to the application or shared library for ZipFS-enabled builds, a platform specific default directory, the directory configured at build time, the directory **lib/tcl***VERSION* under the parent directory of the directory containing the application executable.
...
The **argv0** global variable can be used (in conjunction with the [info script][info] command) to determine whether the current script is being executed as the main script or loaded as a library.  This is useful because it allows a single script to be used as both a library and a demonstration of that library:
