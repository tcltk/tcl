    ...
    TCL_UNUSED(Tcl_Obj *) /*path*/)
{
    ...
    if (retval == NULL) {
	Tcl_PrintfResult(interp, "couldn't create temporary file: %s",
				 ...
				 * process. */
{
	...
	Tcl_PrintfResult(interp, "couldn't create pipe: %s",
		Tcl_PosixError(interp));
	goto error;
    }
	...
	errno = childErrno;
#endif
	Tcl_PrintfResult(interp, "couldn't fork child process: %s",
		Tcl_PosixError(interp));
	goto error;
    }
	...
	Tcl_PrintfResult(interp, "%s: %s", end, Tcl_PosixError(interp));
	goto error;
    }
