...
#define UNSUPPORTED_OPTION(detail) \
    if (interp) {							\
	Tcl_PrintfResult(interp, "%s not supported for this platform",	\
		(detail));						\
    ...
    Tcl_DString *dsPtr)
{
	...
	if (dictObj == NULL) {
	    Tcl_PrintfResult(interp, "couldn't read file channel status: %s",
    ...
    const char *value)		/* New value for option. */
{
	...
	if (strncasecmp(value, "NONE", vlen) == 0) {
	    ...
	    if (interp) {
		Tcl_PrintfResult(interp, "bad value for -handshake: must be one of %s",
	...
	badXchar:
	    if (interp) {
		Tcl_PrintfResult(interp, "bad value for -xchar: should be %s",
			"a list of two elements with each a single 8-bit character");
	...
	if ((argc % 2) == 1) {
	    if (interp) {
		Tcl_PrintfResult(interp, "bad value for -ttycontrol: should be %s",
			"a list of signal,value pairs");
		Tcl_SetErrorCode(interp, "TCL", "OPERATION", "FCONFIGURE",
			"VALUE", (char *)NULL);
	    }
	    Tcl_Free(argv);
	...
	for (i = 0; i < argc-1; i += 2) {
		...
		if (interp) {
		    Tcl_PrintfResult(interp,
			    "bad signal \"%s\" for -ttycontrol: must be %s",
			    argv[i], "DTR, RTS or BREAK");
		    Tcl_SetErrorCode(interp, "TCL", "OPERATION", "FCONFIGURE",
			    "VALUE", (char *)NULL);
		}
		Tcl_Free(argv);
	...
	if (strncasecmp(value, "DEFAULT", vlen) == 0) {
	    ...
	    if (interp) {
		Tcl_PrintfResult(interp, "bad mode \"%s\" for -closemode: must be %s",
			value, "default, discard, or drain");
		Tcl_SetErrorCode(interp, "TCL", "OPERATION", "FCONFIGURE",
			"VALUE", (char *)NULL);
	    }
	    return TCL_ERROR;
	...
	if (strncasecmp(value, "NORMAL", vlen) == 0) {
	    ...
	    if (interp) {
		Tcl_PrintfResult(interp, "bad mode \"%s\" for -inputmode: must be %s",
			value, "normal, password, raw, or reset");
		Tcl_SetErrorCode(interp, "TCL", "OPERATION", "FCONFIGURE",
			"VALUE", (char *)NULL);
	    }
	    return TCL_ERROR;
    ...
    Tcl_DString *dsPtr)		/* Where to store value(s). */
{
	...
	if (ioctl(fsPtr->fileState.fd, TIOCGWINSZ, &ws) < 0) {
	    if (interp != NULL) {
		Tcl_PrintfResult(interp, "couldn't read terminal size: %s",
			Tcl_PosixError(interp));
	    }
	    return TCL_ERROR;
	}
    ...
    TtyAttrs *ttyPtr)		/* Filled with data from mode string */
{
    ...
    if ((i != 4) || (mode[end] != '\0')) {
	if (interp != NULL) {
	    Tcl_PrintfResult(interp, "%s: should be baud,parity,data,stop",
		    bad);
...
#define PARITY_MSG	"n, o, or e"
	...
	if (interp != NULL) {
	    Tcl_PrintfResult(interp, "%s parity: should be %s", bad, PARITY_MSG);
	    Tcl_SetErrorCode(interp, "TCL", "VALUE", "SERIALMODE", (char *)NULL);
	}
	return TCL_ERROR;
    }
	...
	if (interp != NULL) {
	    Tcl_PrintfResult(interp, "%s data: should be 5, 6, 7, or 8", bad);
	    Tcl_SetErrorCode(interp, "TCL", "VALUE", "SERIALMODE", (char *)NULL);
	}
	return TCL_ERROR;
    }
	...
	if (interp != NULL) {
	    Tcl_PrintfResult(interp, "%s stop: should be 1 or 2", bad);
	    Tcl_SetErrorCode(interp, "TCL", "VALUE", "SERIALMODE", (char *)NULL);
	}
	return TCL_ERROR;
    }
    ...
    void **filePtr)		/* Store pointer to FILE structure here. */
{
	...
	Tcl_PrintfResult(interp, "\"%s\" wasn't opened for writing", chanID);
	Tcl_SetErrorCode(interp, "TCL", "VALUE", "CHANNEL", "NOT_WRITABLE",
		(char *)NULL);
	return TCL_ERROR;
    } else if (!forWriting && !(chanMode & TCL_READABLE)) {
	Tcl_PrintfResult(interp, "\"%s\" wasn't opened for reading", chanID);
	...
	if (Tcl_GetChannelHandle(chan,
	    ...
	    if (f == NULL) {
		Tcl_PrintfResult(interp, "cannot get a FILE * for \"%s\"",
			chanID);
    ...
    Tcl_PrintfResult(interp, "\"%s\" cannot be used to get a FILE *", chanID);
