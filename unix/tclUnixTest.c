    ...
    Tcl_Obj *const *objv)	/* Argument strings. */
{
	...
	if (i < 0 || i >= MAX_PIPES) {
	    Tcl_PrintfResult(interp, "bad index %d", i);
	    return TCL_ERROR;
	}
	pipePtr = &testPipes[i];
    }
	...
	Tcl_PrintfResult(interp, "%d %d", pipePtr->readCount, pipePtr->writeCount);
	...
	if (pipePtr->readFile == NULL) {
		...
		Tcl_PrintfResult(interp, "couldn't open pipe: %s",
...
#else
	    Tcl_PrintfResult(interp, "cannot make pipes non-blocking");
	    return TCL_ERROR;
#endif
	}
	pipePtr->readCount = 0;
	...
	if (strcmp(Tcl_GetString(objv[3]), "readable") == 0) {
	    ...
	    Tcl_PrintfResult(interp, "bad read mode \"%s\"", Tcl_GetString(objv[3]));
	    return TCL_ERROR;
	}
	if (strcmp(Tcl_GetString(objv[4]), "writable") == 0) {
	    ...
	    Tcl_PrintfResult(interp, "bad read mode \"%s\"", Tcl_GetString(objv[4]));
	    return TCL_ERROR;
	}
	...
	memset(buffer, 'b', 10);
	int written = write(GetFd(pipePtr->writeFile), buffer, 10);
	...
	if (pipePtr->readFile == NULL) {
	    Tcl_PrintfResult(interp, "pipe %s doesn't exist", Tcl_GetString(objv[2]));
	    return TCL_ERROR;
	}
	if (strcmp(Tcl_GetString(objv[3]), "readable") == 0) {
	    mask = TCL_READABLE;
	...
	Tcl_PrintfResult(interp, "bad option \"%s\": must be %s",
		...
		"close, clear, counts, create, empty, fill, "
    ...
    Tcl_Obj *const *objv)	/* Argument strings. */
{
	...
	Tcl_PrintfResult(interp, "bad argument \"%s\": must be %s",
		Tcl_GetString(objv[2]), "readable, writable, or both");
	return TCL_ERROR;
    }
    if (Tcl_GetChannelHandle(channel,
	    (mask & TCL_READABLE) ? TCL_READABLE : TCL_WRITABLE,
	    (void **) &data) != TCL_OK) {
	Tcl_PrintfResult(interp, "couldn't get channel file");
	return TCL_ERROR;
    }
    fd = (int)PTR2INT(data);
    ...
    Tcl_Obj *const *objv)	/* Argument strings. */
{
    ...
    pid = fork();
    if (pid == -1) {
	Tcl_PrintfResult(interp, "Cannot fork");
	return TCL_ERROR;
    }
    ...
    Tcl_Obj *const *objv)	/* Argument strings. */
{
	...
	Tcl_PrintfResult(interp, "sigaction: %s", Tcl_PosixError(interp));
	return TCL_ERROR;
    }
    (void) alarm(sec);
    return TCL_OK;
    ...
    Tcl_Obj *const *objv)	/* Argument strings. */
{
	...
	if (chmod(Tcl_DStringValue(&ds), (mode_t)mode) != 0) {
	    Tcl_PrintfResult(interp, "%s: %s", translated, Tcl_PosixError(interp));
	    Tcl_DStringFree(&ds);
	    return TCL_ERROR;
	}
